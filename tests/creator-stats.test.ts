...
describe("creatorStats", () => {
  ...
  it("counts trial batches once, in the trend as well as the week", () => {
    ...
    expect(s.current.posts).toBe(1);
    expect(s.totals.posts).toBe(1);
    expect(s.totals.trialUploads).toBe(20);
    // The published winner is what gets celebrated, not a trial upload.
    expect(s.topPosts[0].post.views).toBe(80000);
  });
