...
import { trialUploadShortcodes } from "@/lib/performance";
// groupTrialUploads lives next to transcript matching, not in performance:
// the matcher and the collapse must group a batch identically, so there is
// exactly one copy and both import it from here.
